...
class neuralNet:
        ...
        def __init__(self,acts=7):
                ...
                self.info = None
                self.dqn_solver = DQNSolver(2, acts)
        ...
        def run(self,obs, Done=False):
          ...
          # Printing utility values
                ...
                self.del_utility = self.utility_now - self.utility_prev
                                
          # Printing change in utility values
                print("del_utility: ",self.del_utility)
          ...
          # Printing previous and present states
                ...
                self.index = self.dqn_solver.act(self.state)
                new_cwnd = int(max(obs[6],obs[5]+action[self.index]*(obs[6]*obs[6]/obs[5])))
                new_ssThresh = int(max(2*obs[6], obs[5]/2))  
                exec_action = [new_ssThresh, new_cwnd]
         # Printing cwnd of previous state and present state        
                print("Action taken: ", action[self.index])
                print("cwnd of previous state: ",obs[5])
                print("cwnd of present state: ",new_cwnd)
                
                file = open("CWND_nn_1" + str(obs[0]) + ".txt", "a")
             ...
             # If the line count is even, it means we have a present and next state, so we can do the remember and experience replay. 
                if(self.line_count>= 2):
                        ...
                        self.dqn_solver.remember(state_prev, index_prev, reward_prev, self.state, self.done[0])
